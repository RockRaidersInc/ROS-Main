...
class LaneDetector:
    ...
    def lane_detector(self, input_image, depth_img=None):
        ...
        if self.debug:
            self.color_filtered_pub.publish((bridge.cv2_to_imgmsg(np.dstack([masked_img, masked_img, masked_img]).astype(np.uint8), 'bgr8')))
    ...
    def main(self):
        ...
        self.color_filtered_pub = rospy.Publisher('/output_imaage', Image, queue_size=10)
